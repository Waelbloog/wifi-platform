...
// ----------------------------------------------------
// [جديد] مسار الإضافة اليدوية لكروت مفردة
// ----------------------------------------------------
app.post('/api/cards/manual', async (req, res) => {
    const { network_id, package_id, username, password } = req.body;
    try {
        if (!username || !package_id || !network_id) {
            return res.status(400).json({ success: false, message: 'البيانات غير مكتملة' });
        }

        await pool.query(
            `INSERT INTO cards (network_id, package_id, username, password, status) VALUES ($1, $2, $3, $4, 'available')`,
            [network_id, package_id, username, password || '']
        );

        res.json({ success: true, message: '✅ تم إضافة الكرت للخزنة بنجاح!' });
    } catch (err) {
        console.error(err);
        res.status(500).json({ success: false, message: 'حدث خطأ أثناء الإضافة' });
    }
});

// ----------------------------------------------------
// [جديد] مسار التوليد التلقائي عبر المايكروتك (MikroTik API Wrapper)
// ----------------------------------------------------
app.post('/api/cards/generate-mikrotik', async (req, res) => {
    const { network_id, package_id, quantity, router_ip, router_user, router_pass, profile_name } = req.body;
    
    try {
        // 💡 هنا يتم استخدام مكتبة (node-routeros) في الإنتاج الحقيقي للاتصال بالراوتر
        console.log(`📡 جاري الاتصال براوتر ${router_ip} باليوزر ${router_user}...`);
        
        // محاكاة الاتصال والتوليد (لأغراض التطوير الحالي)
        let generatedCards = 0;
        
        for (let i = 0; i < quantity; i++) {
            // توليد يوزر عشوائي (أرقام فقط أو أحرف) - مثال: 6 أرقام
            const randomPin = Math.floor(100000 + Math.random() * 900000).toString();
            
            /*
            // 💡 كود الإنتاج الحقيقي لإرسال الكرت للمايكروتك:
            await routerOS.write('/ip/hotspot/user/add', [
                `=name=${randomPin}`,
                `=password=${randomPin}`, // إذا كان نظام PIN الباسورد هو نفسه اليوزر
                `=profile=${profile_name || 'default'}`,
                `=server=all`
            ]);
            */

            // حفظ الكرت المتولد في قاعدة بيانات "ميتا ترون" لكي يُعرض في المتجر
            await pool.query(
                `INSERT INTO cards (network_id, package_id, username, password, status) VALUES ($1, $2, $3, $4, 'available')`,
                [network_id, package_id, randomPin, '']
            );
            generatedCards++;
        }
        ...
        res.json({ 
            success: true, 
            message: `🤖 تم توليد ${generatedCards} كرت بنجاح في المايكروتك وحفظها في المتجر!` 
        });

    } catch (err) {
        console.error('MikroTik API Error:', err);
        res.status(500).json({ success: false, message: '❌ فشل الاتصال بالمايكروتك. تأكد من الـ IP والصلاحيات.' });
    }
});
// ----------------------------------------------------
// 13. نظام التذاكر والدعم الفني (Support System)
// ----------------------------------------------------

// أ. إنشاء تذكرة شكوى جديدة (يستدعيها العميل من حسابه)
app.post('/api/tickets', async (req, res) => {
    const { customer_phone, network_id, subject, description } = req.body;
    try {
        // البحث عن العميل
        const customerResult = await pool.query('SELECT id FROM customers WHERE phone = $1', [customer_phone]);
        const customer_id = customerResult.rows.length > 0 ? customerResult.rows[0].id : null;

        await pool.query(
            `INSERT INTO support_tickets (customer_id, network_id, subject, description, status) 
             VALUES ($1, $2, $3, $4, 'OPEN')`,
            [customer_id, network_id, subject, description]
        );
        res.json({ success: true, message: 'تم استلام تذكرتك بنجاح، سيقوم الدعم الفني بمراجعتها فوراً.' });
    } catch (err) {
        console.error('Ticket Error:', err);
        res.status(500).json({ success: false, message: 'فشل إرسال التذكرة' });
    }
});

// ب. جلب التذاكر لموظف الدعم (يستدعيها موظف الدعم)
app.get('/api/support/tickets', async (req, res) => {
    try {
        const tickets = await pool.query(`
            SELECT t.id, t.subject, t.description, t.status, t.created_at, 
                   c.phone as customer_phone, n.network_name
...
// ج. تحديث حالة التذكرة من قبل موظف الدعم (مثال: من OPEN إلى RESOLVED)
